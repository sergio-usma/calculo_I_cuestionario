...
// ------------------------------------------------------------
// CARGA DE EJERCICIOS DESDE JSON (con manejo de errores localStorage)
// ------------------------------------------------------------
async function cargarEjercicios() {
  try {
    ...
    // Cargar estado de completados desde localStorage con try-catch
    try {
      const stored = localStorage.getItem("completados");
      ejerciciosCompletados = stored ? JSON.parse(stored) : [];
    } catch (e) {
      console.warn("No se pudo acceder a localStorage (lectura):", e);
  ...
  try {
    ...
    if (ex.tipo === "quad") {
      if (ex.h !== undefined) {
        ...
        return x1 === x2
          ? `(${x1.toFixed(1)},0)`
          : `(${x1.toFixed(1)},0) y (${x2.toFixed(1)},0)`;
      } else {
  ...
  try {
    switch (ex.tipo) {
      ...
      case "elipse":
        let [h2, k2] = ex.centro;
        return (
          Math.abs((x - h2) ** 2 / ex.a ** 2 + (y - k2) ** 2 / ex.b ** 2 - 1) <
          0.001
        );
      case "racional":
...
function normalizarOpcion(opt) {
  ...
  let intervaloInfMatch = raw.match(
    /^([\[\(])\s*(-?(?:\d+\.?\d*|∞))\s*,\s*(-?(?:\d+\.?\d*|∞))\s*([\]\)])$/,
  );
...
function generarPreguntasParaEjercicio(ex) {
  ...
  function crearOpciones(baseOptions, correctValue) {
    ...
    let correctIndex = opciones.findIndex(
      (opt) => normalizarOpcion(opt) === normalizedCorrect,
    );
  ...
  if (ex.tipo === "producto") {
    try {
      ...
      let { opciones: op1, correcta: corr1 } = crearOpciones(
        base1,
      ...
      let base5 = ["Sí", "No"];
      let { opciones: op5, correcta: corr5 } = crearOpciones(
        base5,
        aSubB ? "Sí" : "No",
      );
      preguntas.push({
        ...
        explicacion: aSubB
          ...
          : "Hay elementos de A que no están en B.",
      });
      ...
      let base6 = ["V", "F"];
      let { opciones: op6, correcta: corr6 } = crearOpciones(
        base6,
        igual ? "V" : "F",
      );
      preguntas.push({
        ...
        correcta: corr6,
        explicacion: igual
          ? "Los conjuntos son iguales porque A = B."
          : "El producto cartesiano no es conmutativo, salvo que A = B.",
      });

      let esFunc = B.length === 1;
      let base7 = ["V", "F"];
      let { opciones: op7, correcta: corr7 } = crearOpciones(
        base7,
        esFunc ? "V" : "F",
      );
      preguntas.push({
        ...
        explicacion: esFunc
          ...
          : "Cada elemento de A se relaciona con varios elementos de B, por lo que no es función.",
      });
      ...
      let { opciones: op8, correcta: corr8 } = crearOpciones(
        base8,
        String(A.length),
      );
      preguntas.push({
    ...
    let { opciones: opDom, correcta: corrDom } = crearOpciones(
      opcionesDom,
      dom,
    );
    preguntas.push({
    ...
    let { opciones: opRan, correcta: corrRan } = crearOpciones(
      opcionesRan,
      ran,
    );
    preguntas.push({
    ...
    preguntas.push({
      ...
      explicacion:
        func === 0
          ? "Cada x tiene una única imagen."
          : "Hay valores de x con dos imágenes posibles.",
    });

    // Pregunta 6: Vértice (solo para cuadráticas)
    if (vertice) {
      let opcionesVert = [vertice, "(0,0)", "No tiene vértice", "(1,1)"];
      let { opciones: opVert, correcta: corrVert } = crearOpciones(
        opcionesVert,
        vertice,
      );
      preguntas.push({
    ...
    if (asintota) {
      ...
      let { opciones: opAsint, correcta: corrAsint } = crearOpciones(
        opcionesAsint,
        asintota,
      );
      preguntas.push({
    ...
    if (discriminante !== null) {
      ...
      let { opciones: opDisc, correcta: corrDisc } = crearOpciones(
        opcionesDisc,
        discVal,
      );
      let regla =
    ...
    if (centro) {
      ...
      let { opciones: opCentro, correcta: corrCentro } = crearOpciones(
        opcionesCentro,
        centro,
      );
      preguntas.push({
    ...
    if (radioSemiejes) {
      ...
      let { opciones: opRadio, correcta: corrRadio } = crearOpciones(
        opcionesRadio,
        radioSemiejes,
      );
      preguntas.push({
    ...
    if (repr && ex.tipo !== "circunferencia" && ex.tipo !== "elipse") {
      ...
      let { opciones: opRepr, correcta: corrRepr } = crearOpciones(
        opcionesRepr,
        repr,
      );
      preguntas.push({
        texto:
          "Selecciona la representación por comprensión correcta de la función:",
        opciones: opRepr,
        correcta: corrRepr,
        explicacion:
          "La notación correcta debe incluir la ecuación y el dominio y rango adecuados.",
      });
    }
    ...
    if (ex.tipo === "lineal") puntoNotable = { x: 0, y: ex.b };
    ...
    } else if (ex.tipo === "sqrt")
      puntoNotable = { x: ex.a, y: ex.desplazamientoV || 0 };
      ...
      puntoNotable = {
        x: ex.centro[0] + (ex.tipo === "circunferencia" ? ex.radio : ex.a),
        y: ex.centro[1],
      };
    ...
    if (tieneMaximo(ex)) {
      ...
      preguntas.push({
        ...
        correcta: corr14,
        explicacion:
          "El coeficiente principal es negativo, por lo que la parábola abre hacia abajo y tiene un máximo en el vértice.",
      });
      ...
      preguntas.push({
        ...
        correcta: corr14,
        explicacion:
          "El coeficiente principal es positivo, por lo que tiene un mínimo.",
      });
    }
    ...
    if (crec !== null) {
      let base15 = ["V", "F"];
      let { opciones: op15, correcta: corr15 } = crearOpciones(
        base15,
        crec ? "V" : "F",
      );
      preguntas.push({
        ...
        correcta: corr15,
        explicacion: crec
          ? "La pendiente es positiva (o la función es creciente)."
    ...
    if (ex.tipo === "sqrt") {
      ...
      preguntas.push({
        ...
        correcta: corr16,
        explicacion:
          "No, porque \\(y^2 = ...\\) representa dos ramas, mientras que la función raíz cuadrada da solo la rama positiva.",
      });
    }
    ...
    let perteneceDom =
      dom.includes("ℝ") ||
      (dom.includes("[") &&
        xTest >= parseFloat(dom.split(",")[0].replace("[", ""))) ||
      (dom.includes("]") &&
        xTest <= parseFloat(dom.split(",")[1].replace("]", ""))) ||
      (dom.includes("(") &&
        xTest > parseFloat(dom.split(",")[0].replace("(", ""))) ||
      (dom.includes(")") &&
        xTest < parseFloat(dom.split(",")[1].replace(")", "")));
    let base17 = ["Sí", "No"];
    let { opciones: op17, correcta: corr17 } = crearOpciones(
      base17,
      perteneceDom ? "Sí" : "No",
    );
    preguntas.push({
    ...
    let perteneceRan =
      ran.includes("ℝ") ||
      (ran.includes("[") &&
        yTest >= parseFloat(ran.split(",")[0].replace("[", ""))) ||
      (ran.includes("]") &&
        yTest <= parseFloat(ran.split(",")[1].replace("]", ""))) ||
      (ran.includes("(") &&
        yTest > parseFloat(ran.split(",")[0].replace("(", ""))) ||
      (ran.includes(")") &&
        yTest < parseFloat(ran.split(",")[1].replace(")", "")));
    let base18 = ["Sí", "No"];
    let { opciones: op18, correcta: corr18 } = crearOpciones(
      base18,
      perteneceRan ? "Sí" : "No",
    );
    preguntas.push({
    ...
    if (ex.tipo === "circunferencia" || ex.tipo === "elipse") {
      ...
      let base19 = ["Sí", "No"];
      let { opciones: op19, correcta: corr19 } = crearOpciones(
        base19,
        simX ? "Sí" : "No",
      );
      preguntas.push({
        ...
        correcta: corr19,
        explicacion: simX
          ? "Sí, porque el centro está sobre el eje X."
          : "No, porque el centro no está sobre el eje X.",
      });
    }
    ...
    if (ex.tipo === "exponential") {
      ...
      preguntas.push({
        ...
        correcta: corr20,
        explicacion:
          "Las funciones exponenciales son monótonas y no tienen máximos ni mínimos locales.",
      });
    }
  }
...
function cargarEjercicio() {
  ...
  if (completedBadge) {
    if (completado) {
      ...
      completedBadge.style.display = "inline-block"; // Refuerzo
    } else {
  ...
  const btnVerify = document.querySelectorAll(
    '.btn-success[onclick="verificarTodo()"]',
  );
  if (completado) {
...
// ------------------------------------------------------------
// NUEVA FUNCIÓN PARA MARCAR COMO COMPLETADO (con refuerzo visual y logs)
// ------------------------------------------------------------
function marcarComoCompletado() {
  console.log("Ejecutando marcarComoCompletado");
  const completedBadge = document.getElementById("completedBadge");
  if (completedBadge) {
    // Eliminar cualquier clase que pueda ocultarlo
    completedBadge.classList.remove("d-none");
    // Forzar estilo inline con prioridad
    completedBadge.style.setProperty("display", "inline-block", "important");
    console.log("Badge mostrado. Clases actuales:", completedBadge.className);
    console.log("Estilo computed display:", window.getComputedStyle(completedBadge).display);
  } else {
    console.error("Badge no encontrado en el DOM");
  }

  // Deshabilitar todos los radios y botones de verificar
  const radios = document.querySelectorAll('input[type="radio"]');
  const btnVerify = document.querySelectorAll('.btn-success[onclick="verificarTodo()"]');
  radios.forEach((r) => (r.disabled = true));
  btnVerify.forEach((b) => (b.disabled = true));
  console.log("Radios y botones deshabilitados");
}

// ------------------------------------------------------------
// VERIFICAR TODO (con la llamada a marcarComoCompletado)
// ------------------------------------------------------------
function verificarTodo() {
  ...
  preguntasActuales.forEach((p, idx) => {
    ...
    if (!feedback || !item) {
      console.error(
        `No se encontró el elemento de retroalimentación para la pregunta ${idx}`,
      );
      return;
    }
  ...
  if (correctas === total && !ejerciciosCompletados.includes(idx)) {
    ...
    guardarCompletados(); // Intenta guardar, pero no interrumpe si falla
    marcarComoCompletado(); // Muestra el badge y deshabilita elementos
  }
  ...
  try {
    ...
    if (ex.tipo === "sqrt_frac") {
      ...
      if ((ex.reflexion && x1 < ex.a) || (!ex.reflexion && x1 > ex.a)) {
        ...
        puntos.push({
          x: x1,
          y: y1,
          label: `(${x1.toFixed(2)},${y1.toFixed(2)})`,
        });
      }
      return puntos;
    }
    ...
    if (ex.tipo === "quad") {
      if (ex.h !== undefined) {
        ...
        puntos.push({
          x: xv,
          y: yv,
          label: `Vértice (${xv.toFixed(2)},${yv.toFixed(2)})`,
        });
        if (ex.a !== 0) {
          let disc = -ex.k / ex.a;
          if (disc > 0) {
        ...
        if (disc > 0) {
          ...
          puntos.push({
            x: (-b - sqrtDisc) / (2 * a),
            y: 0,
            label: `(${((-b - sqrtDisc) / (2 * a)).toFixed(2)},0)`,
          });
          puntos.push({
            x: (-b + sqrtDisc) / (2 * a),
            y: 0,
    ...
    if (ex.tipo === "exponential") {
      ...
      puntos.push({
        x: 1,
        y: ex.base + desp,
        label: `(1,${(ex.base + desp).toFixed(2)})`,
      });
      let cx = obtenerCorteX(ex);
...
function generarGrafica() {
    ...
    try {
      switch (ex.tipo) {
        ...
        case "sqrt":
          ...
          if (argumento >= 0)
            y = Math.sqrt(argumento) + (ex.desplazamientoV || 0);
          break;
        case "sqrt_frac":
          let argF = ex.reflexion ? ex.a - x : x - ex.a;
          if (argF > 0)
            y = ex.num / Math.sqrt(argF) + (ex.desplazamientoV || 0);
          break;
        case "racional":
          let argR = ex.reflexion ? ex.a - x : x - ex.a;
  ...
  dataPrincipal = dataPrincipal.filter(
    (p) => p.y >= limitesY.min && p.y <= limitesY.max,
  );
  dataNegativa = dataNegativa.filter(
    (p) => p.y >= limitesY.min && p.y <= limitesY.max,
  );
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            label: (item) =>
              `(${item.parsed.x.toFixed(1)}, ${item.parsed.y.toFixed(1)})`,
          },
        },
      },
    },
...
function initDarkMode() {
  ...
  const iconElements = [
    ...
    document.querySelector("#darkModeFloating i"),
  ];
...
window.onload = function () {
  ...
  document
    ...
    .addEventListener("change", cargarEjercicio);
};